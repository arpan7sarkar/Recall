...
export function StarButton({
  ...
  duration = 3,
  lightColor = "#FAFAFA",
  backgroundColor = "var(--button-star-fill)",
  borderWidth = 1,
  className,
  ...props
}: StarButtonProps) {
  ...
  React.useEffect(() => {
    ...
    const updatePath = () => {
      ...
      pathRef.current.style.setProperty(
        "--path",
        `path('M 0 0 H ${pathRef.current.offsetWidth} V ${pathRef.current.offsetHeight} H 0 V 0')`
      );
    };
  ...
  return (
    <button
      ...
      className={cn(
        "relative z-[3] inline-flex h-10 items-center justify-center gap-2 overflow-hidden whitespace-nowrap rounded-3xl px-4 py-2 text-sm font-medium transition-all duration-200 disabled:pointer-events-none disabled:opacity-50",
        className
      )}
      {...props}
    >
      ...
      <span className="relative z-10 font-semibold text-[var(--button-primary-text)]">
        {children}
      </span>
    </button>
  );
