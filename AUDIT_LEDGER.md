...
| RUN-004 | High | Fixed | API and worker health endpoints report process uptime without checking PostgreSQL, Redis, queues, worker liveness, or whether the application schema exists. | API `/ready` queries migrated user, extension-token, and current item fields before checking Redis and BullMQ queues; worker `/ready` checks Redis; `/live` remains process liveness. | `recall-runtime`, `recall-migration-readiness` | API readiness tests pass 2/2 and runtime contract tests pass 7/7; live public model query verified after migration deploy |
...
| AUTH-008 | Critical | Fixed | Vercel had the production API URL configured, but the browser bundle still saw it as undefined because the client passed the whole `process.env` object instead of statically analyzable public-variable reads. | The API client now captures every supported public API variable through a direct `process.env.NEXT_PUBLIC_*` expression. | `recall-vercel-env` | Production build with `NEXT_PUBLIC_RENDER_API_URL` succeeds and emits the configured URL in a client chunk; focused configuration tests pass 4/4 |
...
| ID | Severity | Status | Problem | Evidence | Target branch | Verification |
|---|---|---|---|---|---|---|
| UI-001 | High | Fixed | Dashboard and archive exposed only the first 20 items with no pagination or infinite loading. | `useInfiniteItems` consumes the API page/totalPages contract, flattens loaded pages, and dashboard/archive render visible Load more controls with next-page loading and retry states. | `recall-pagination-consistency` | `apps/web/tests/dashboard-pagination.test.tsx` passes; integrated web suite 55/55 and web typecheck pass |
...
| DATA-003 | Critical | Fixed | The configured production Neon database was reachable but its `public` schema had zero tables and all four repository migrations were pending, causing Clerk user synchronization and every model query to fail while readiness passed. | All four migrations were deployed to the validated empty schema; the API start script now runs `prisma migrate deploy`, and readiness queries current application tables and fields. | `recall-migrations`, `recall-migration-readiness` | `prisma migrate status` reports up to date; live public collection probe changed from HTTP 500 to expected 404; API 58/58 and runtime 7/7 pass |
