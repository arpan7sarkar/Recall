...
|---|---|---|---|---|---|---|
| RUN-001 | Critical | Confirmed | The root `npm run dev` command starts only the web app, leaving the API and worker stopped. | `package.json:4-6`; README requires three separate processes. | `recall-runtime` | Not started |
...
| RUN-004 | High | Confirmed | API and worker health endpoints report process uptime without checking PostgreSQL, Redis, queues, or worker liveness. | `apps/api/src/index.ts:72-79`; `apps/api/src/workers/index.ts:11-27`. | `recall-readiness` | Not started |
...
| RUN-009 | High | Confirmed | Worker health starts before Redis is validated and remains healthy during Redis failure. | `apps/api/src/workers/index.ts:11-36`. | `recall-readiness` | Not started |
| RUN-010 | High | Confirmed | The API catches uncaught exceptions and deliberately keeps the process alive in an unknown state. | `apps/api/src/index.ts:117-120`. | `recall-runtime` | Not started |
...
| AUTH-004 | High | Confirmed | Default CORS accepts only frontend port 3000, so Next falling back to port 3001 breaks every browser API request unless an undocumented variable is configured. | `apps/api/src/index.ts:18-48`; incomplete `apps/api/.env.example`. | `recall-config` | Not started |
...
| TEST-004 | High | Confirmed | Root scripts do not orchestrate build, lint, test, API, worker, or dependency services. | Root `package.json`. | `recall-runtime` | Not started |
| TEST-005 | High | Confirmed | API environment template omits required authentication, CORS, readiness, and public storage variables. | `apps/api/.env.example`. | `recall-config` | Not started |
